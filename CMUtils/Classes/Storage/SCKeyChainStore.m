...
NSString * const SCKeyChainStoreErrorDomain = @"com.kishikawakatsumi.SCKeyChainStore";
static NSString *_defaultService;

@interface SCKeyChainStore ()

@end

@implementation SCKeyChainStore
...
+ (SCKeyChainStore *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (SCKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:SCKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = SCKeyChainStoreItemClassGenericPassword;
...
{
    return [self initWithServer:server protocolType:protocolType authenticationType:SCKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(SCKeyChainStoreProtocolType)protocolType authenticationType:(SCKeyChainStoreAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = SCKeyChainStoreItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = SCKeyChainStoreAccessibilityAfterFirstUnlock;
}
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    SCKeyChainStore *keychain = [SCKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
+ (NSArray UIC_KEY_TYPE *)allKeysWithItemClass:(SCKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == SCKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == SCKeyChainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(SCKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == SCKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == SCKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'SCKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'SCKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case SCKeyChainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case SCKeyChainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case SCKeyChainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case SCKeyChainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case SCKeyChainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case SCKeyChainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case SCKeyChainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case SCKeyChainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case SCKeyChainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case SCKeyChainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case SCKeyChainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case SCKeyChainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case SCKeyChainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case SCKeyChainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case SCKeyChainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case SCKeyChainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case SCKeyChainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case SCKeyChainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case SCKeyChainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case SCKeyChainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case SCKeyChainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case SCKeyChainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case SCKeyChainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case SCKeyChainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case SCKeyChainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case SCKeyChainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case SCKeyChainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case SCKeyChainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case SCKeyChainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case SCKeyChainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case SCKeyChainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case SCKeyChainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case SCKeyChainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case SCKeyChainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case SCKeyChainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case SCKeyChainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case SCKeyChainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case SCKeyChainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case SCKeyChainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case SCKeyChainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case SCKeyChainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case SCKeyChainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case SCKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case SCKeyChainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case SCKeyChainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case SCKeyChainStoreAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:SCKeyChainStoreErrorDomain code:SCKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:SCKeyChainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:SCKeyChainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:SCKeyChainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
@implementation SCKeyChainStore (Deprecation)
